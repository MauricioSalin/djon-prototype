        ...
        <div className="grid lg:grid-cols-2 gap-12 items-center">
          ...
          >
            <div>
              ...
              >
                {data.title.split("\n").map((line, index) => (
                  <span key={`${index}:${line}`} className="block" style={index === 0 ? undefined : { color: "var(--djon-color-accent)", WebkitTextStroke: "2px var(--djon-color-ink)", paintOrder: "stroke fill" }}>{line}</span>
                ))}
              </motion.h2>
            </div>
            ...
            >
              {data.items.map((item) => (
                ...
                >
                  ...
                  <p className="text-djon-text/60 text-xs mt-1 leading-relaxed">{item.description}</p>
                </motion.div>
              ))}
            </motion.div>
              ...
              <div className="absolute bottom-0 left-0 right-0 flex items-center gap-2 bg-djon-black/90 px-3 py-3 backdrop-blur-sm sm:gap-3 sm:px-4">
                <div className="min-w-0">
                  ...
                  <p className="text-djon-accent text-djon-label mt-0.5">
                    {units.find((unit) => unit.key === selectedLocation)?.openingHours ?? "Seg – Sex · 9h às 18h"}
                  </p>
                </div>
                <motion.a
