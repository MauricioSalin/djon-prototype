...
import { store, type Lead, type Unit } from "@/lib/store"
...
export default function LeadsAdminPage() {
  ...
  const remove = async (lead: Lead) => {
    const name = [lead.firstName, lead.lastName].filter(Boolean).join(" ") || lead.whatsapp || lead.email || "Contato"
    ...
    <main className="mx-auto max-w-5xl space-y-6 px-4 py-8 sm:px-6 sm:py-10">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
        ...
        <DjonSelect value={filter} onChange={(value) => setFilter(value as typeof filter)}
          options={[{ value: "todos", label: "Todos" }, ...statuses.map((status) => ({ value: status, label: statusLabels[status] }))]}
        ...
        <article key={lead.id} className="rounded-2xl border border-djon-text/8 bg-djon-surface-2 p-5">
          <div className="flex flex-col gap-4 lg:flex-row lg:items-start">
            <div className="min-w-0 flex-1">
              ...
              {lead.whatsapp ? (
                <a href={whatsappUrl(lead.whatsapp)} target="_blank" rel="noopener noreferrer" className="mt-1 flex items-center gap-2 text-xs font-bold text-djon-accent"><Phone size={12} />{formatPhone(lead.whatsapp)}</a>
              ) : lead.email ? (
              ...
              {lead.unitKey && <p className="mt-2 flex items-center gap-2 text-xs text-djon-text/40"><MapPin size={12} />{unitLabel(lead.unitKey)}</p>}
            ...
            <div className="w-full space-y-3 lg:w-72">
              <DjonSelect value={lead.status} onChange={(status) => updateLocal(lead.id, { status: status as Lead["status"] })}
                ...
                ariaLabel={`Status do contato de ${lead.firstName || lead.whatsapp || lead.email || "visitante"}`} />
