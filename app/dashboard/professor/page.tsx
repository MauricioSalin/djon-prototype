...
export default function ProfessorHomePage() {
  ...
  useEffect(() => {
    ...
    void store.bootstrap().then(async (u) => {
      ...
      if (u.role !== "professor") {
        router.replace(
          u.role === "admin" ? "/dashboard/admin" : "/dashboard/student",
        );
        return;
      }
  ...
  const quickLinks = [
    ...
    {
      label: "Meus Eventos",
      desc: "Divulgue onde você vai tocar",
      href: "/dashboard/professor/evento",
      icon: Music2,
    },
    {
      label: "Alunos",
      desc: "Veja os perfis dos alunos",
      href: "/dashboard/professor/alunos",
      icon: Users,
    },
    {
      label: "Professores",
      desc: "Equipe da DJ ON Academy",
    ...
    <div className="bg-djon-page">
      ...
      <section className="py-16 bg-djon-muted-panel sm:py-20">
        <div className="max-w-7xl mx-auto px-4 sm:px-6">
          ...
          <motion.div
            className="h-[3px] w-10 bg-djon-accent rounded-full mb-10"
            {...fadeUp(0.15)}
          />
          ...
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
            {[
              ...
              <motion.div
                ...
                whileHover={{ y: -4 }}
              >
                <p className="text-5xl font-black tracking-tighter mb-2 text-djon-accent">
                  {s.value}
                </p>
                <p className="text-djon-text/40 text-xs font-bold tracking-widest uppercase">
                  {s.label}
                </p>
            ...
            {quickLinks.map((q, i) => (
              ...
              >
                ...
                >
                  ...
                  <div>
                    <p className="text-djon-text font-black text-base tracking-tight group-hover:brightness-110 transition-colors">
                      {q.label}
                    </p>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6">
          <div className="flex items-end justify-between flex-wrap gap-4 mb-10">
            <div>
              ...
              <motion.div
                className="h-[3px] w-10 bg-djon-accent rounded-full mt-3"
                {...fadeUp(0.15)}
              />
            </div>
          ...
          {upcoming.length === 0 ? (
            ...
            >
              ...
              <p className="text-djon-text/30 text-sm font-bold">
              ...
              {upcoming.map((b, i) => {
                ...
                return (
                  ...
                  >
                    <div className="flex items-center justify-between mb-3">
                      <div className="flex items-center gap-2">
                        <div className="djon-avatar-fallback w-8 h-8 rounded-full flex items-center justify-center shrink-0">
                          <span className="text-djon-accent font-black text-xs">
                            {studentName.charAt(0)}
                          </span>
                        </div>
                        <div>
                          <p className="text-djon-text text-xs font-black">
                            {studentName}
                          </p>
                          <p className="text-djon-text/30 text-djon-label capitalize">
                            {b.type}
                          </p>
                        </div>
                      </div>
                      <span
                        className={`inline-flex items-center gap-1.5 rounded-full border px-2.5 py-1 text-djon-label font-black tracking-widest ${status.badge}`}
                      >
                        <span
                    ...
                    <h3 className="text-djon-text font-black text-base tracking-tight mb-3 leading-tight">
                      {b.title}
                    </h3>
                    <div className="flex items-center gap-2 text-djon-text/40 text-xs">
                      <Clock size={11} />
                      {fmt(b.date)} às {b.time}
                    </div>
                  </motion.button>
                );
              })}
