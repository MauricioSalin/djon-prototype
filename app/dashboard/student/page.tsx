...
export default function StudentPage() {
  ...
  const upcoming = bookings
    .filter(
      (b) =>
        b.status !== "cancelado" &&
        b.status !== "recusado" &&
        new Date(b.date + "T00:00:00") >= new Date(),
    )
  ...
  return (
    <div className="bg-djon-page">
      ...
      <EditablePortalHero
        heroKey="student-home"
        defaults={STUDENT_HOME_HERO}
      ...
      <UpcomingEventsSection background="muted" />
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6">
          ...
          {myEvents.length === 0 ? (
            <motion.div className="rounded-2xl border-2 border-dashed border-djon-text/10 p-8 text-center sm:p-16" {...fadeUp(0.2)}>
              ...
              <p className="text-djon-text/30 text-sm font-bold">Nenhum evento cadastrado</p>
            </motion.div>
          ) : (
            <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
              {sortedMyEvents.map((ev, i) => {
                ...
                return (
                  <motion.div
                    ...
                    initial={{ opacity: 0 }}
                    whileInView={{ opacity: isPast ? 0.5 : 1 }}
                    viewport={{ once: true, amount: 0 }}
                    transition={{ delay: i * 0.07, duration: 0.6, ease: [0.25, 0.4, 0.25, 1] as const }}
                    whileHover={{ y: -4 }}
                  >
