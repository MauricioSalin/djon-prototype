...
export default function RedefinirSenhaPage() {
  return (
    <div className="min-h-svh flex items-center justify-center px-4 py-10 relative overflow-hidden noise-overlay bg-djon-page">
      ...
      <motion.div className="relative z-10 w-full max-w-md" initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }}>
        <div className="mb-10 flex justify-center"><Image src="/images/djon-verde.png" alt="DJ ON Academy" width={221} height={56} className="h-14 w-auto" priority /></div>
        <div className="rounded-2xl border border-djon-text/10 bg-djon-surface-2 p-8"><p className="mb-2 text-xs font-bold tracking-wide text-djon-accent">SEGURANÇA</p><h1 className="mb-2 text-3xl font-black tracking-tighter text-djon-text">Crie uma nova senha</h1><p className="mb-8 text-sm leading-relaxed text-djon-text/45">Use pelo menos 8 caracteres e escolha uma senha que só você conhece.</p><Suspense fallback={<p className="text-sm text-djon-text/50">Carregando...</p>}><ResetPasswordForm /></Suspense></div>
      </motion.div>
    </div>
  )
}
